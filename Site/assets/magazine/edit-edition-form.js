...
function saveEdit() {
    ...
    var publicationNo = document.getElementById('updatePublicationNo').value;
    var publicationPlace = document.getElementById('updatePublicationPlace').value;
    var publisherName = document.getElementById('updatePublisherName').value;
    ...
    if (publicationNo < 1 || publicationNo == null) {
        document.getElementById('updatePublicationNo').style.border = '1px solid red';
        document.getElementById('updatePublicationNo').nextElementSibling.style.color = "red";
    }
    ...
    if (editionNo < 1 || editionNo == null || segmentNo < 1 || publicationNo < 1 || publicationQuantity < 1 || publicationPages < 10 || publicationYear == 0 || registrationYear == 0 || registrationMonth == 0 || registrationDay == 0) {
        ...
        $.ajax({
            type: "POST",
            url: "/Magazine/EditEditionAndSegment",
            data: { bookAndMagazine: bookAndMagazine, segment: segment, edition: edition, currentSegmentNo: currentSegmentNo, currentEditionNo: currentEditionNo },
            success: function (response) {
                ...
                if (response == "updated") {
                    ...
                    document.getElementById('TDPublicationNo' + editionId).innerHTML = document.getElementById('updatePublicationNo').value;
                    document.getElementById('TDPublicationPlace' + editionId).innerHTML = document.getElementById('updatePublicationPlace').value;
                    document.getElementById('TDPublisherName' + editionId).innerHTML = document.getElementById('updatePublisherName').value;
...
function savePublicationEdit() {
    ...
    if (name == "" || authors == "") {
        ...
        $.ajax({
            type: "POST",
            url: "/Magazine/EditPublication",
            data: { bookProperties: bookProperties, authors: authors },
            success: function (response) {
...
function addSegment() {
    ...
    var publicationNo = document.getElementById('addPublicationNo').value;
    var publicationPlace = document.getElementById('addPublicationPlace').value;
    var publisherName = document.getElementById('addPublisherName').value;
    ...
    if (publicationNo < 1 || publicationNo == null) {
        document.getElementById('addPublicationNo').style.border = '1px solid red';
        document.getElementById('addPublicationNo').nextElementSibling.style.color = "red";
    }
    if (publicationQuantity < 1 || publicationQuantity == null) {
    ...
    // Display alert if form was not validate
    if (editionNo < 1 || editionNo == null || segmentNo < 1 || publicationNo < 1 || publicationQuantity < 1 || publicationPages < 10 || publicationYear == 0 || registrationDate == null || registrationDate == "") {
...
function EditEdition(s_no, edition_id, segment_id, edition_no, segment_no, publication_no, publication_place, publisher_name, publication_quantity, publication_pages, cd_quantity, publication_date_type, publication_year, publication_month, publication_day, registration_date_type, registration_year, registration_month, registration_day, cupboard_no, cell_no) {
    showTab("tab3");
    
    var publicationDataType = "";
    var RegistrationDataType = "";

    if (publication_date_type == " شمسي ") {
        publicationDataType = "شمسي";
    }
    else if (publication_date_type = " قمري ") {
        publicationDataType = "قمري";
    }
    else {
        publicationDataType = "میلادي";
    }

    if (registration_date_type == " شمسي ") {
        RegistrationDataType = "شمسي";
    }
    else if (registration_date_type = " قمري ") {
        RegistrationDataType = "قمري";
    }
    else {
        RegistrationDataType = "میلادي";
    }
    ...
    document.getElementById('updatePublicationNo').value = publication_no;
    document.getElementById('updatePublicationPlace').value = publication_place;
    document.getElementById('updatePublisherName').value = publisher_name;
    ...
    document.getElementById('updatePublicationDateType').value = publicationDataType;
    ...
    document.getElementById('updateRegistrationDateType').value = RegistrationDataType;
