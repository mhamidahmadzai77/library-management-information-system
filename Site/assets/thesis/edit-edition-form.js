...
function saveEdit() {
    ...
    if (editionNo < 1 || editionNo == null || segmentNo < 1 || publicationQuantity < 1 || publicationPages < 10 || publicationYear == 0 || registrationYear == 0 || registrationMonth == 0 || registrationDay == 0) {
        ...
        $.ajax({
            type: "POST",
            url: "/Thesis/EditEditionAndSegment",
...
function savePublicationEdit() {
    ...
    var internalConflict = $('#internalConflict').val();
    var externalConflict = $('#externalConflict').val();
    ...
    if (internalConflict == "" || internalConflict == null) {

        document.getElementById('internalConflict').style.color = "red";
        document.getElementById('internalConflict').nextElementSibling.style.color = "red";
    }

    if (externalConflict == "" || externalConflict == null) {

        document.getElementById('externalConflict').style.color = "red";
        document.getElementById('externalConflict').nextElementSibling.style.color = "red";
    }

    if (name == "" || authors == "" || supervisorFirstname == "" || internalConflict == "" || externalConflict == "" ) {
        ...
        var thesis = {
            ...
            external_conflict: externalConflict
        };
        ...
        $.ajax({
            type: "POST",
            url: "/Thesis/EditPublication",
            data: { bookProperties: bookProperties, monographAndThesis: monographAndThesis, thesis: thesis, authors: authors },
            success: function (response) {
